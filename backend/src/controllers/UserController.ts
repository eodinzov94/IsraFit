...
class UserController {
    ...
    async registration(req: TypedRequestBody<IUserRegister>, res: Response, next: NextFunction) {
        const { email, birthYear, gender, lastName, firstName, password, weight, height, physicalActivity } = req.body
            ...
            const user = await User.create(
                {
                    ...
                    weight, height, physicalActivity
                })
    ...
    async updateUser(req: TypedRequestBody<IUserUpdate>, res: Response, next: NextFunction) {
        ...
        try {
            ...
            return res.json({
                status: 'OK', user: getUserData(user)
            })
        } catch (e: any) {
