...
describe('BMI chart tests', function() {
  let driver
  ...
  describe('find the canvas', function() {
    it('should find the canvas', async function() {
      await driver.wait(until.elementLocated(By.xpath('//a[@href="/bmi-chart"]')), 5000)
      await driver.findElement(By.xpath('//a[@href="/bmi-chart"]')).click();
      assert.strictEqual(await driver.getCurrentUrl(), 'http://localhost:5173/bmi-chart', 'URL did not change!');
